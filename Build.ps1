$nugetExe = if(Test-Path Env:NuGet) { Get-Content env:NuGet } else { Join-Path .\ "nuget\nuget.exe" }
...
"@ | Out-File $NuSpec
    . $nugetExe pack $NuSpec -OutputDirectory .\Packages -NonInteractive -Version $($moduleData.version)
}